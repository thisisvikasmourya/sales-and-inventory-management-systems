...
class Main(splash,Login,Admin,User):
    ...
    def buildmain(self):
        ...
        self.topframe=LabelFrame(self.mainw,width=1400,height=120,bg="#2b5aa4")
        self.topframe.place(x=0,y=0)
        self.store_name = 'THE'
        self.storelable=Label(self.topframe,text=self.store_name + " SALES & INVENTORY SYSTEM ",bg="#2b5aa4",anchor="center")
        self.storelable.config(font="Roboto 30 bold",fg="snow")
        self.storelable.place(x=360,y=30)
        mi = PhotoImage(file="images/admin12.png")
        mi = mi.subsample(4,4)
