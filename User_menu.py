...
class User:
    ...
    def user_mainmenu(self,a,b):
        self.mainframe = LabelFrame(self.mainw, width=800, height=140, bg="#4267b2")
        self.mainframe.place(x=330, y=120)
        mi = PhotoImage(file="images/add.png")
        mi = mi.subsample(a, b)
        self.aitems = Button(self.mainframe, text="Items",bd=2,font="roboto 11 bold",bg="#4267b2", image=mi, compound=TOP,command=self.builditemtable)
        self.aitems.image = mi
        self.aitems.place(x=260, y=17)
        mi = PhotoImage(file="images/invoice2.png")
        mi = mi.subsample(a,b)
        self.aitems = Button(self.mainframe, text="Invoice",bd=2,bg="#4267b2",font="roboto 11 bold", image=mi, compound=TOP,command=self.make_invoice)
        self.aitems.image = mi
        self.aitems.place(x=62, y=17)
        mi = PhotoImage(file="images/change1.png")
        mi = mi.subsample(a, b)
        self.changeuser = Button(self.mainframe, text="Sign out",bd=2,bg="#4267b2",font="roboto 11 bold", image=mi, compound=TOP)
        self.changeuser.image = mi
        self.changeuser.place(x=460, y=17)
        mi = PhotoImage(file="images/Door_Out-512.png")
        mi = mi.subsample(a, b)
        self.logout = Button(self.mainframe, text="Quit",bd=2,bg="#4267b2",font="roboto 11 bold", image=mi, compound=TOP)
        self.logout.image = mi
        self.logout.place(x=670, y=17)
        self.tableframe1 =Frame(self.mainw, width=150, height=400,bg="#4267b2")
        self.tableframe1.place(x=1230, y=270, anchor=NE)
        self.tableframe1info=self.tableframe1.place_info()
        self.tableframe =Frame(self.mainw, width=350, height=700,bg="#4267b2")
        self.tableframe.place(x=1110, y=300, anchor=NE)
        self.tableframeinfo = self.tableframe.place_info()
        self.entryframe = Frame(self.mainw, width=800, height=350, bg="#4267b2")
        self.entryframe.place(x=810, y=460+20)
        self.entryframeinfo = self.entryframe.place_info()
        self.entryframe1 = Frame(self.mainw, width=500, height=350, bg="#4267b2")
    ...
    def user_input(self):
       ...
       Button(self.entryframe,text="Proceed",command=self.transtableadd,bd=10,width=8,height=7,bg="#4267b2",font="roboto 10").place(x=0,y=30)
       Button(self.entryframe, text="Add to cart", command=self.addtotrans, bd=10, width=10, height=3,bg="#4267b2",font="roboto 10").place(x=100,y=80)
       Button(self.entryframe, text="Remove", command=self.removecart, bd=10, width=10, height=3, bg="#4267b2",font="roboto 10").place(x=210, y=80)
       ...
       cartqty = Entry(self.entryframe,textvariable=self.qty,width=9,bg="#4267b2",font="roboto 12")
       cartqty.place(x=320,y=30,height=30)
       carttotal = Entry(self.entryframe, textvariable=self.total, width=20, state='readonly', bg="#4267b2", font="roboto 12")
       carttotal.place(x=130, y=185, height=60)
       Label(self.entryframe, text="Quantity", font="roboto 12 bold", bg="#4267b2").place(x=318,y=0)
       Label(self.entryframe, text="Search", font="roboto 12 bold", bg="#4267b2").place(x=100, y=0)
       Label(self.entryframe, text="Amount Due", font="roboto 14 bold", bg="#4267b2").place(x=0, y=205)
       self.cur.execute("select max(invoice) from sales")
       self.invoice = self.cur.fetchall()
       self.invoice = self.invoice[0][0] + 1
       Label(self.tableframe1, text="Invoice No. "+str(self.invoice), font="roboto 14 bold", bg="#4267b2").grid(row=0, column=0)
       ...
       va=0
       for i in range(0,4):
           Label(self.entryframe1, text=li[i], font="roboto 14 bold", bg="#4267b2").place(x=0, y=va)
           va += 65
       self.cartitemid = StringVar()
       self.cartitem = StringVar()
       self.cartitemprice = StringVar()
       self.cartitemstock = StringVar()
       Entry(self.entryframe1, textvariable=self.cartitemid, font="roboto 14", bg="#4267b2", width=25, state='readonly').place(x=162, y=0,height = 40)
       Entry(self.entryframe1, textvariable=self.cartitem, font="roboto 14", bg="#4267b2", width=25, state='readonly').place(x=162, y=65,height=40)
       Entry(self.entryframe1, textvariable=self.cartitemprice, font="roboto 14", bg="#4267b2", width=25, state='readonly').place(x=162,y=65*2,height=40)
       Entry(self.entryframe1, textvariable=self.cartitemstock, font="roboto 14", bg="#4267b2", width=25, state='readonly').place(x=162,y=65*3,height=40)
       self.id_qty=dict()
    ...
    def makeprint(self):
       if messagebox.askyesno("Alert!","Print this transaction?")== True:
           ...
           if not os.path.exists(dir):
               os.makedirs(dir)
           shop="\t\t\t\t\t  The Sales & Inventory Management System\n"
           address="\t\t\t\t Malad Mumbai 400095 Maharashtra\n"
